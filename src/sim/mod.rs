...
impl Segment {
    ...
    pub fn new_with_curvature(
        ...
        let phi = if d0 != d1 { (d1 - d0) / l } else { 0.0 };
        let r0 = RHO * C / a0;
...
pub fn create_segments_from_geo_with_curvature(
    ...
    for window in geo.windows(2) {
        ...
        let d0 = d0_mm / 1000.0;
        let d1 = d1_mm / 1000.0;
        segs.push(Segment::new_with_curvature(
            x0,
            x1,
            d0,
            d1,
...
/// Compute complex wavenumber with viscothermal losses (simplified model).
///
/// This is a legacy simplified boundary-layer approximation.
/// For DidgeLab-aligned formulation, use `viscothermal_loss_params`.
...
impl DidgeridooSimulator {
    ...
    pub fn impedance(&self, freqs: &[f64]) -> Vec<Complex<f64>> {
        match self.strategy {
            SimulationStrategy::Tlm => freqs
                .iter()
                .map(|&f| {
                    cadsd_ze_with_losses(
                        &self.segments,
                        f,
...
mod tests {
    ...
    fn test_insert_toneholes_single() {
        ...
        let elements = insert_toneholes(&segments, &toneholes);
        let tonehole_count = elements
            .iter()
            .filter(|e| matches!(e, TlmElement::Tonehole(_)))
            .count();
    ...
    fn test_cadsd_ze_with_losses() {
        ...
        let segments = create_segments_from_geo(&geo.geo);
        let z_lossy =
            cadsd_ze_with_losses(&segments, 440.0, &AcousticConstants::default(), true, &[]);
        let z_clean =
            cadsd_ze_with_losses(&segments, 440.0, &AcousticConstants::default(), false, &[]);
    ...
    fn test_full_pipeline_geo_to_optimizer() {
        ...
        let spectrum: Vec<Complex<f64>> = freqs
            .iter()
