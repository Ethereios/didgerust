...
use cadsd::evo::{Genome, KigaliGenome, LossFunction};
use cadsd::sim::{
    find_resonance_peaks, AcousticConstants, DidgeridooSimulator, SimulationStrategy,
};
...
fn test_full_workflow_cone() {
    ...
    let result2 = plot_evolution_progress(
        &vec![0, 1, 2],
        &vec![1.0, 0.5, 0.2],
        &vec![1.0, 0.7, 0.4],
        &format!("{}/progress.png", output_dir),
    );
...
fn test_geometry_to_simulation_workflow() {
    ...
    assert!(
        !impedances.is_empty(),
        "Impedance spectrum should not be empty"
    );
...
fn test_tlm_python_simulation() {
    ...
    assert!(
        !impedances.is_empty(),
        "TLM impedance spectrum should not be empty"
    );
    assert_eq!(impedances.len(), freqs.len());
}
...
fn test_tonehole_impedance() {
    ...
    assert!(
        z_open.norm() > 0.0,
        "Open tonehole impedance should be positive"
    );

    let z_closed = th.closed_impedance(440.0, &constants);
    assert!(
        z_closed.norm() > 0.0,
        "Closed tonehole impedance should be positive"
    );
}
...
fn test_simulator_with_acoustic_constants() {
    ...
    assert!(
        !spectrum.is_empty(),
        "Simulator spectrum should not be empty"
    );
...
fn test_tonehole_comparison_workflow() {
    ...
    assert!(
        differences > 0,
        "Toneholes should change impedance spectrum"
    );
...
fn test_tonehole_edge_tone_resistance() {
    ...
    assert_eq!(
        r_closed, 0.0,
        "Closed tonehole should have zero edge-tone resistance"
    );
    ...
    assert!(
        r_open >= 0.0,
        "Open tonehole edge-tone resistance should be non-negative"
    );
}
...
fn test_tonehole_edge_tone_frequency_dependence() {
    ...
    assert!(
        r_440 >= r_100,
        "Edge-tone resistance should increase with frequency in low range"
    );
    assert!(
        r_1000 >= 0.0,
        "Edge-tone resistance should be non-negative at high frequency"
    );
}
...
fn test_tonehole_edge_tone_diameter_dependence() {
    ...
    assert!(
        r_small > r_medium,
        "Smaller holes should have higher edge-tone resistance"
    );
    assert!(
        r_medium > r_large,
        "Medium holes should have higher resistance than large holes"
    );
}
...
fn test_waveguide_with_toneholes() {
    use cadsd::{
        sim::{AcousticConstants, DidgeridooSimulator, SimulationStrategy},
        tonehole::Tonehole,
    };
    let geo = Geo::make_cone(1000.0, 32.0, 60.0, 20);
    ...
    for z in spec.iter() {
        assert!(
            z.norm() > 0.0,
            "Waveguide tonehole impedance should be finite"
        );
    }
}
...
fn test_complex_impedance_strategy() {
    use cadsd::sim::{AcousticConstants, DidgeridooSimulator, SimulationStrategy};
    let geo = Geo::make_cone(1000.0, 32.0, 60.0, 20);
    let mut sim =
        DidgeridooSimulator::with_strategy(&geo.geo, SimulationStrategy::ComplexImpedance);
    sim.acoustic_constants = AcousticConstants::for_temperature(20.0);
...
fn test_default_optimizer() {
    ...
    assert!(
        !geo.geo.is_empty(),
        "Optimized geometry should not be empty"
    );
    assert!(
        geo.length() > 0.0,
        "Optimized geometry should have positive length"
    );
}
...
fn test_validate_fdtd_vs_tlm() {
    ...
    let (fdtd_z, tlm_z, rel_err) =
        validate_fdtd_vs_tlm(&geo, 200.0, &cadsd::sim::AcousticConstants::default());
...
fn test_bent_effective_length_cli() {
    ...
    assert!(
        d_l < ds,
        "Bent effective length should be less than geometric length"
    );
...
fn test_bent_effective_length_zero_curvature() {
    ...
    assert!(
        (d_l - ds).abs() < 1e-12,
        "Zero curvature should give same length"
    );
}
