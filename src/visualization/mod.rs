...
use crate::sim::{grid, DidgeridooSimulator, Resonance};
use crate::Geo;
...
pub fn plot_bore_geometry(geo: &Geo, output_path: &str) -> Result<(), Box<dyn Error>> {
    ...
    let points: Vec<(f64, f64)> = geo.geo.iter().map(|p| (p[0], p[1])).collect();
...
pub fn plot_impedance_spectrum(geo: &Geo, output_path: &str) -> Result<(), Box<dyn Error>> {
...
pub fn plot_evolution_progress(
    ...
    let max_loss = best_losses
        .iter()
        .chain(avg_losses.iter())
        .cloned()
    ...
    let best_data: Vec<(u32, f64)> = generations
        .iter()
        .zip(best_losses.iter())
        .map(|(&g, &l)| (g as u32, l))
        .collect();

    let avg_data: Vec<(u32, f64)> = generations
        .iter()
        .zip(avg_losses.iter())
        .map(|(&g, &l)| (g as u32, l))
        .collect();
...
pub fn create_analysis_report(geo: &Geo, output_dir: &str) -> Result<(), Box<dyn Error>> {
    use std::fs;
